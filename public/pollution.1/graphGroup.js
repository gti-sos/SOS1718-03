   ...
   $http.get(api).then(function(response1) {
    $http.get(apiPropia).then(function(response2) {
     ...
     for (var i = 0; i < response1.data.length; i++) {
      ...
      ultimoAux.push({ name: response1.data[i].solarPlant });
      ultimoAux.push({ name: response1.data[i].peakPower });
      ultimoAux.push({ name: response1.data[i].temperature });
      var tdDetail = { name: response1.data[i].name.toUpperCase().substring(3, 0), children: ultimoAux };
      children.push(tdDetail)
     }
     for (var i = 0; i < response2.data.length; i++) {
      ultimoAux = [];
      ultimoAux.push({ name: parseInt(response2.data[i].year) });
      ultimoAux.push({ name: response2.data[i].station });
      ultimoAux.push({ name: response2.data[i].nitrous });
      ultimoAux.push({ name: response2.data[i].car });
